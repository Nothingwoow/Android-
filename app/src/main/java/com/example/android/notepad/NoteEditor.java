    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        switch (item.getItemId()) {
            case R.id.menu_save:
                ...
                updateNote(text, null);
                // 发送广播
                Intent intent1 = new Intent("com.example.notes.UPDATE_LIST");
            ...
            case R.id.menu_delete:
                deleteNote();
                // 发送广播
                Intent intent2 = new Intent("com.example.notes.UPDATE_LIST");
                sendBroadcast(intent2);
                finish();
                break;
            case R.id.menu_revert:
                // 发送广播
                Intent intent3 = new Intent("com.example.notes.UPDATE_LIST");
                sendBroadcast(intent3);
                cancelNote();
                break;
            case R.id.menu_change_note_bg_color:
                // 更改笔记背景颜色
    ...
    private void changeNoteBackgroundColor() {
        ...
        builder.setItems(colorNames, new DialogInterface.OnClickListener() {
            ...
            public void onClick(DialogInterface dialog, int which) {
                applyNoteBackgroundColor(colors[which]);
            }
        });
        builder.create().show();
    }

    private void applyNoteBackgroundColor(int color) {
        // 改变整个Activity背景颜色
